...
describe('Phase 3-5 UI + structure', () => {
  ...
  it('perf: Lottie dynamic, no web3 LAMPORTS barrel in UI', () => {
    for (const f of [
      'app/components/button/ThanksBtn.tsx',
      'app/components/web3/Deposit.tsx',
      'app/components/web3/Withdraw.tsx',
    ]) {
      assert.match(read(f), /dynamic\(.*lottie-player/i);
      // UI must define its own unit constant; only a named LAMPORTS import
      // from web3 is banned (namespace use for Transaction/PublicKey is fine).
      assert.doesNotMatch(
        read(f),
        /import\s*\{[^}]*LAMPORTS_PER_SOL[^}]*\}\s*from\s*['"]@solana\/web3\.js['"]/,
      );
      assert.match(read(f), /const LAMPORTS_PER_SOL = 1_000_000_000/);
    }
  });
});
