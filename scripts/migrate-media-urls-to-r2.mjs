...
async function getCurrentMedia(client) {
  const videos = await client.query('SELECT id, "thumbnailUrl", "videoUrl" FROM "Video" ORDER BY id');
  const channels = await client.query('SELECT id, image FROM "Channel" WHERE image IS NOT NULL ORDER BY id');
  const userImages = await client.query('SELECT image FROM "User" WHERE image IS NOT NULL ORDER BY id');
...
function validateSourceMedia(media, targetBaseURL) {
  ...
  if (media.channels.length !== EXPECTED_COUNTS.channelLogos) {
    throw new Error(`Expected ${EXPECTED_COUNTS.channelLogos} channel logos, found ${media.channels.length}`);
  }
  ...
  for (const video of media.videos) {
    objectKeys.add(
      assertMediaURL(video.thumbnailUrl, SOURCE_MEDIA_URL, 'thumbnail', `Video ${video.id} thumbnail`),
    );
    objectKeys.add(assertMediaURL(video.videoUrl, SOURCE_MEDIA_URL, 'temp-video', `Video ${video.id} video`));
  }

  for (const channel of media.channels) {
    objectKeys.add(
      assertMediaURL(channel.image, SOURCE_MEDIA_URL, 'channel-logo', `Channel ${channel.id} image`),
    );
  }

  if (
    objectKeys.size !==
    EXPECTED_COUNTS.thumbnails + EXPECTED_COUNTS.videos + EXPECTED_COUNTS.channelLogos
  ) {
...
async function applyMigration(client, backup) {
  ...
  try {
    const thumbnails = await client.query(
      ...
      [backup.targetBaseURL, SOURCE_MEDIA_URL.length + 1, `${SOURCE_MEDIA_URL}/%`],
    );
    ...
    const migratedChannelsById = new Map(migratedMedia.channels.map((channel) => [channel.id, channel]));
  ...
  try {
    for (const video of backup.videos) {
      const result = await client.query(
        ...
        [video.thumbnailUrl, video.videoUrl, video.id, video.targetThumbnailUrl, video.targetVideoUrl],
      );
