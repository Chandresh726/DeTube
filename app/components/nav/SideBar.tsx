...
const SidebarChannel = ({ id, image, name, sidebarState }: { id: number; image?: string | null; name: string; sidebarState: string }) => (
    <Button asChild variant="ghost" className={cn('w-full justify-start', sidebarState === 'icons' && 'justify-center px-2')} title={name}>
        <Link
            href={`/channel/${id}`}
