      ...
      <TableBody>
        {rows.map((transaction) => (
          <TableRow key={transaction.id}>
            ...
            <TableCell>{formatDTSol(Number(transaction.amount))} SOL</TableCell>
            <TableCell className="hidden lg:table-cell">
...
const Statement = ({ session }: { session: { user: { id: number | string } } }) => {
  const [statement, setStatement] = useState<StatementData | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    let cancelled = false;
    const getStatement = async (id: number | string) => {
      if (!id) return;
      try {
        setError(null);
